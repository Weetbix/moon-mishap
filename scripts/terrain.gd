...
var TERRAIN_X_POINTS: int = 65
var TERRAIN_X_SPACING: int = 10
var TERRAIN_HEIGHT: float = 400.0
var TERRAIN_Y_BASIS: float = 300.0

func match_line_to_collision() -> void:
	line.clear_points()
...
func find_smoothest_point(points: PackedVector2Array, invalid_point_indexes: Array) -> int:
	var smoothest_index := 1
	var smoothest_value: float = 10000
	...
	for i in range(1, points.size() - 3):
		...
		var is_valid_index := !invalid_point_indexes.any(
			func(index: int) -> int: return (i > index - INVALID_POINT_BUFFER)&&(i < index + INVALID_POINT_BUFFER)
		)

		if is_valid_index:
			var difference_in_neighours: float = abs(points[i].y - points[i - 1].y) + abs(points[i].y - points[i + 1].y)
...
func find_jankiest_point(points: PackedVector2Array, invalid_point_indexes: Array) -> int:
	var jankiest_index := 1
	var jankiest_value: float = -1000
...
func make_neighbours_flat(points: PackedVector2Array, index: int) -> void:
	points[index - 1].y = points[index].y
	points[index + 1].y = points[index].y

func remove_all_platforms() -> void:
	for child in platforms.get_children():
		platforms.remove_child(child)

func create_platform(terrain: PackedVector2Array, index: int, color: Color) -> void:
